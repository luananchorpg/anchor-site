...
const services = [
  { label: "Traditional PM", title: "Long-term rental management", body: "Tenant placement, rent collection, maintenance, and renewals — handled end to end. You collect rent, we handle everything else.", href: "/traditional-pm/how-it-works", linkLabel: "How it works →" },
  { label: "Vacation Rentals", title: "Short-term rental management", body: "Listed on Airbnb, VRBO, and direct booking. Dynamic pricing, guest management, and professional cleaning after every stay.", href: "/vacation-rentals", linkLabel: "See vacation rentals →" },
];
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 border-t border-black/10">
        <FadeUp>
          <h2 className="text-3xl md:text-4xl font-extrabold text-black mb-3">Two services. One team.</h2>
          ...
          {services.map((s, i) => (
            <FadeUp key={s.label} delay={i * 100}>
              <div className="card-hover border border-black/10 rounded-2xl p-7 h-full flex flex-col">
                ...
                <Link href={s.href} className="text-sm font-bold text-black hover:opacity-70 transition-opacity">{s.linkLabel}</Link>
              </div>
            </FadeUp>
          ))}
        </div>
